...
TablaHash tablahash_crear(unsigned capacidad, FuncionCopiadora copia,
  ...
  for (unsigned idx = 0; idx < capacidad; ++idx) {
    tabla->elems[idx].dato = glist_crear();
  }
...
void tablahash_destruir(TablaHash tabla) {
  ...
  for (unsigned idx = 0; idx < tabla->capacidad; ++idx)
    if (tabla->elems[idx].dato != NULL)
      glist_destruir(tabla->elems[idx].dato, tabla->destr);
 ...
 */
void tablahash_inserta_encadenamiento(TablaHash tabla, void *dato) {
  // Calculamos la posicion del dato dado, de acuerdo a la funcion hash.
  unsigned idx = tabla->hash(dato) % tabla->capacidad;
  ...
  // Si hay colision agrega el elemento a la lista enlazada de la casilla.
  tabla->elems[idx].dato = glist_agregar_inicio(tabla->elems[idx].dato, dato, tabla->copia);
  return; 
}
